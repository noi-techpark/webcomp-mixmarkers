...
import { fetchWebcams, fetchWeatherForecast, fetchDistricts, fetchMunicipality } from './api/api.js';
...
class OpendatahubWebcams extends HTMLElement {
    ...
    async addSearchInput(){
        ...
        await this.fetchMunicipality('Id,Shortname,IstatNumer,GpsInfo');    
        const mymunicipality = this.municipality;
        const ourmap = this.map;
        ...
        hiddenref.addEventListener("change", function(event) {
                ...
                let result2 = mymunicipality.find(o => o['Id'] === searchref.value);
                ...
                if(result){                    
                    ...
                    const newgps2 = [result["GpsPoints.position"].Latitude, result["GpsPoints.position"].Longitude];
                    ...
                    var newMarker = new L.marker(newgps2, { icon: markericon }).addTo(ourmap);

                    mymap.flyTo(newgps, 13);
                    ourmap.flyTo(newgps2, 13);
